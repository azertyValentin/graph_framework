...
public class DirectedGraph extends AbstractListGraph<DirectedNode> implements IDirectedGraph {
    ...
    public void addArc(DirectedNode from, DirectedNode to) {
        if(!isArc(from,to)){
            getNodeOfList(from).getSuccs().put(getNodeOfList(to), 0);
            getNodeOfList(to).getPreds().put(getNodeOfList(from), 0);
        }
    }
    ...
    public IDirectedGraph computeInverse() {
        ...
        for (DirectedNode n : g.getNodes()) {
            Map<DirectedNode, Integer> preds = g.getNodeOfList(n).getPreds();
            g.getNodeOfList(n).setPreds(n.getSuccs());
            g.getNodeOfList(n).setSuccs(preds);
        }
        return g;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Inversed Matrix: "+new AdjacencyMatrixDirectedGraph(al.computeInverse()));
        System.out.println(new AdjacencyMatrixDirectedGraph(al.toAdjacencyMatrix()));
        System.out.println("Should be false: "+al.isArc(new DirectedNode(2), new DirectedNode(5)));
        al.addArc(new DirectedNode(2), new DirectedNode(5));
        System.out.println("Should be true: "+al.isArc(new DirectedNode(2), new DirectedNode(5)));
